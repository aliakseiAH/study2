...
//Пешка  1//Конь   3//Слон   3.5//Ладья  5//Ферзь  9//Король 100
//Строковые обозначения шахматных фигур приведены в таблице:
//Пешка (белая)  ♙//Конь (белый)   ♘//Слон (белый)   ♗//Ладья (белая)  ♖//Ферзь (белый)  ♕//Король (белый) ♔//Пешка (черная)  ♟
//Конь (черный)   ♞//Слон (черный)   ♝//Ладья (черная)  ♜//Ферзь (черный)  ♛//Король (черный) ♚
...
public class Task1 {
    public static void main(String[] args) {
        List<ChessPiece> chess1 = new ArrayList<>();
        for (int i = 0; i < 4; i++) {
            chess1.add(ChessPiece.PAWN_WHITE);
        }
        for (int i = 0; i < 4; i++) {
            chess1.add(ChessPiece.ROOK_BLACK);
        }
        for (int i = 0; i < 8; i++) {
            System.out.print(chess1.get(i).getName() + " ");
        }
